...
Self-hosted SMTP/IMAP email integration for Precept CRM. Supports multi-account configuration, compose & send, inbox sync, threaded conversations, and bulk campaigns with template rendering.
...
Shared volume: dkim_keys
  ...
  ├── {domain}/mail.private ← DKIM private key (auto-generated)
  └── {domain}/mail.txt     ← DKIM public key (for DNS record)
```

**Configuration flow:**
1. User sets `smtp_mode = "builtin"` and enters `mail_domain` in the UI
2. Django writes `postfix_config.json` to the shared `dkim_keys` volume
3. Postfix entrypoint reads the config file (polls every 10s if not yet available)
4. Postfix generates DKIM keys and starts accepting mail for that domain
5. User retrieves DKIM public key via `/dkim-record` endpoint and adds DNS records
...
| `GET`    | `/dkim-record`                | Read DKIM public key from shared volume, return `{selector, domain, dns_name, record}` |
...
When using built-in SMTP mode, three DNS records are required for email deliverability:

| Record | Type | Name | Value |
|--------|------|------|-------|
| **SPF** | TXT | `@` | `v=spf1 ip4:<SERVER_IP> ~all` |
| **DKIM** | TXT | `mail._domainkey` | Retrieved from `GET /dkim-record` endpoint |
| **DMARC** | TXT | `_dmarc` | `v=DMARC1; p=none; rua=mailto:dmarc@yourdomain.com` |

The DKIM key is auto-generated by the Postfix container on first startup. The public key can be retrieved from the UI (Settings → Email → DNS Setup panel) or via the API.
...
| File | Purpose |
|------|---------|
| `Dockerfile` | Ubuntu 22.04 + postfix + opendkim + opendkim-tools |
| `entrypoint.sh` | Reads domain from config file or env, generates DKIM keys, configures Postfix + OpenDKIM, starts services |
| `opendkim.conf` | OpenDKIM base configuration (relaxed/simple, inet:8891) |

**Docker Compose additions:**
- `postfix` service with `dkim_keys` volume mounted at `/etc/opendkim/keys`
- `dkim_keys` volume also mounted read-only on backend/celery-worker for DKIM record access
